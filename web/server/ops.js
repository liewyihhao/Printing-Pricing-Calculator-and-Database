...
function saveConfig(patch, actor) {
  const c = config();
  ['hubs', 'outlets', 'machines', 'couriers', 'productionSite'].forEach(k => { if (patch[k] !== undefined) c[k] = patch[k]; });
...
function normalizeJob(j) {
  ...
  if (!j.hub) { j.hub = hubForDestination(j.finalDestination); changed = true; }
...
function syncOrder(oid) {
  ...
  else if (any(['scheduling', 'printing', 'outsourcing', 'logistics', 'dispatched', 'at_hub', 'ready_collect', 'completed'])) p = 'in_production';
...
function afterTransition(j, from, to, action, actor, payload) {
  ...
  if (action === 'dispatch' || action === 'vendor_ship' || action === 'forward') ship(payload.courier, payload.tracking);
  if (action === 'receive_hub' || action === 'receive_outlet' || action === 'deliver') { const last = (j.shipments || [])[j.shipments.length - 1]; if (last && !last.receivedAt) { last.receivedAt = now(); last.receivedBy = actor; } }
  ...
  if (to === 'rejected') custNotify(j, 'artwork_issue', 'We need a fix on your artwork', 'Our prepress team found an issue with ' + j.product + ': ' + (payload.reason || 'see details') + '. Please send a corrected file and we’ll check it straight away.');
  if (to === 'dispatched') {
...
function sendInternal(jid, role, actor, body) {
  const j = store.job(jid); if (!j) return { error: 'Job not found' };
  normalizeJob(j);
  const type = body.destType || (j.finalDestination || {}).type || 'customer';
  if (['hub', 'outlet', 'customer'].indexOf(type) < 0) return { error: 'Delivery must be to a hub, an outlet or the customer.' };
  if (!body.machine) return { error: 'Pick the machine / production line.' };
  const prev = { destination: j.destination, instructions: j.instructions };
  j.destination = destOf(type, body.destId, j, store.order(j.orderId));
  if (type !== 'customer' && body.destId && type === 'hub') j.hub = j.destination.id;
  j.instructions = String(body.instructions || '').slice(0, 600);
...
function award(jid, role, actor, body) {
  ...
  // where the printer sends it: a hub by default (consolidation), or straight to the outlet/customer
  const type = body.destType || 'hub';
  j.destination = destOf(type, body.destId, j, store.order(j.orderId));
