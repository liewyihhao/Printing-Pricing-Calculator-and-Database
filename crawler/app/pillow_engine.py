"""Pillow (Litho) pricing engine. Fixed spec (standard sublimation pillow). Drivers: qty(10-190).

  cash_price(qty) -> RM
  build_params() -> writes output/pillow_params.json
"""
...
FILE = OUT / "pillow_samples.json"
PARAMS = OUT / "pillow_params.json"
TIER_DISCOUNTS = {"Cash": 0.0, "Silver": 0.04, "Gold": 0.08, "Platinum": 0.14}
WEIGHT_FACTOR = 1.2065
# Standard 40cm × 40cm sublimation pillow with filling ~600g
PILLOW_KG = 0.60
_CACHE: dict = {}
...
def weight_kg(qty: int) -> float:
    return round(PILLOW_KG * float(qty) * WEIGHT_FACTOR, 3)
...
def loo_audit() -> dict:
    ...
    e.sort()
    return {"pillow": round(e[len(e)//2], 2)}
