"""Sample Wall Calendar (Litho) prices. Fixed spec (Size 260x265mm, Boxboard Grey Back
300gsm backing + Simili 60gsm 12-sheet content, Folding + Side Stitching compulsory).
Only driver: QUANTITY. Saves output/wallcal_samples.json: {"core":[{qty,cash}]}.

  python -m app.wallcal_sampler [account]
"""
...
URL = "https://www.excard.com.my/spec/Litho/Wall_Calendar"
QTYS = [1000, 2000, 3000, 4000, 5000, 7000, 10000, 14000, 18000]


async def run(account_id=1):
    a = accounts.get(account_id)
    out = OUT / "wallcal_samples.json"
    ...
    async with async_playwright() as pw:
        ...
        for q in QTYS:
            ...
            if c and c != prev:
                ...
                if c and c != prev:
                    data["core"].append({"qty": q, "cash": c}); prev = c
            out.write_text(json.dumps(data, indent=0)); log.info("wc.q", q=q, cash=c)
        try: await b.close()
